...
- Pago Movil and Transferencia Directa run as provider-authoritative flows with `/verifications/pago-movil/*` and `/verifications/transferencia-directa/*`. They use per-company InstaPago transport configuration (`proxy` by default, `direct` when credentials are authorized for the LedgerLink host), validate VES payments by date, and persist every provider attempt in `PaymentProviderVerificationAttempt`.
- `/companies/:companySlug/verifications/capture/extract` is a read-only helper for external POS clients. It receives a temporary image data URL plus an explicit method profile (`zelle`, `binance`, `pago_movil`, or `transferencia_directa`), extracts structured fields with the vision model, compares detected amount against the expected amount when supplied, and returns only normalized fields, warnings, confidence, extraction id, and image hash. It does not authorize, consume, or persist a payment.
